...
def main():
    ...
    # Training settings
    num_pinn_pretrain_steps = 5000 # Steps to train PINN on data
    num_param_optim_steps = 2000   # Steps to optimize material params
    ...
    # Start with incorrect material parameters
    ...
    # # Use the TRUE material parameters for verification
    # true_material_params = MaterialParameters(E_init=E_true, nu_init=nu_true)
    # verification_model = PINN(model_key) # Use a fresh model

    # # Briefly train a PINN to fit the ground truth data using the true material params
    # # This gives us a differentiable function that represents the FEM solution
    # optimizer_verify = optax.adam(1e-3)
    # params_to_train, static_params = eqx.partition((verification_model, true_material_params), eqx.is_array)
    # opt_state_verify = optimizer_verify.init(params_to_train)

    # for step in range(1001):
    #     current_trainable = eqx.combine(params_to_train, static_params)
    #     (loss_val, _), grads = eqx.filter_value_and_grad(calculate_data_loss, has_aux=True)(
    #         current_trainable, (None, None, None, data_points), (0., 0., 1.)
    #     )
    #     updates, opt_state_verify = optimizer_verify.update(grads, opt_state_verify, params_to_train)
    #     params_to_train = eqx.apply_updates(params_to_train, updates)
    #     if step % 500 == 0:
    #         print(f"  Verification pre-fit step {step}, Data Loss: {loss_val:.2e}")

    # # Now, check the physics loss of this data-fitted model
    # verification_model = eqx.combine(params_to_train, static_params)[0]
    ...
    #     (verification_model, true_material_params), verification_batch, (1., 1., 0.)
    # )
    # print(f"--- Verification Complete ---")
    # print(f"  Physics Loss on Ground Truth: {physics_loss_val:.2e} (PDE: {pde_loss:.2e}, BC: {bc_loss:.2e})")
    # if physics_loss_val > 1e-2:
    #     print("WARNING: Physics loss on ground truth data is high. The FEM data may not be a good representation of the physics.")
